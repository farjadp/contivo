  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-3xl border border-gray-200 p-8 shadow-sm">
         <h2 className="text-xl font-bold tracking-tight text-[#121212] mb-6">{t('mapTitle')}</h2>
         ...
         <div dir="ltr" className="relative w-full h-[300px] sm:h-[400px] border-l-2 border-b-2 border-gray-200 bg-gray-50/50 rounded-tr-lg rounded-bl-lg overflow-visible">
           <span className="absolute -left-12 top-1/2 -translate-y-1/2 -rotate-90 text-[10px] font-bold text-gray-400 uppercase tracking-widest whitespace-nowrap">{t('axisY')}</span>
           <span className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-[10px] font-bold text-gray-400 uppercase tracking-widest whitespace-nowrap">{t('axisX')}</span>
           ...
           <div className="absolute left-[50%] top-[50%] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center">
             ...
             <span className="mt-2 text-xs font-bold text-indigo-700 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded shadow-sm">{t('yourBrand')}</span>
           </div>
         ...
         <div className="mt-10 flex flex-wrap items-center justify-center gap-6 text-xs font-semibold text-gray-500">
           <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-rose-500 border border-rose-200" /> {t('legendDirect')}</div>
           <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-amber-400 border border-amber-200" /> {t('legendIndirect')}</div>
           <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-emerald-400 border border-emerald-200" /> {t('legendAspirational')}</div>
         </div>
      </div>
           ...
           <div key={c.id} className={`bg-white rounded-2xl border p-5 shadow-sm transition-all duration-300 ${c.userDecision === 'REJECTED' ? 'border-red-200 opacity-50 bg-red-50/20 grayscale' : 'border-gray-200 hover:border-gray-300 hover:shadow-md'}`}>
              ...
              {c.userDecision === 'ACCEPTED' && (
                <div className="pt-4 border-t border-gray-100">
                  <label className="text-[10px] uppercase font-bold text-gray-400 tracking-wider mb-2 block">{t('typeLabel')}</label>
                  <select 
                    value={c.type}
                    onChange={(e) => handleType(c.id, e.target.value)}
                    className="w-full bg-gray-50 border border-gray-200 rounded-lg text-xs font-semibold text-[#121212] px-3 py-2.5 outline-none focus:border-indigo-500 focus:bg-white transition-colors cursor-pointer appearance-none [background-position:right_12px_center] rtl:[background-position:left_12px_center]"
